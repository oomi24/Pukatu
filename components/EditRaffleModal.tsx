...
const EditRaffleModal: React.FC<EditRaffleModalProps> = ({ show, onClose, raffleItem, onSave }) => {
  ...
  const resetFormFields = useCallback(() => {
    if (raffleItem) {
      ...
      setEditablePrizeDetails(['']); setEditableGridNumbersTotal(100); setEditableSoldNumbersStr('');
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isNaN(parsedGridTotal) || parsedGridTotal <= 0 || parsedGridTotal > 500) { setErrorMessage("Total Cuadrícula: número >0, <=500."); return; }
    ...
    let parsedSoldNumbers: number[] | undefined = editableSoldNumbersStr.split(',')
        ...
        .filter(n => !isNaN(n) && n > 0 && n <= parsedGridTotal);
      ...
      <div className="bg-[#1e1442] text-white border border-purple-700/50 rounded-lg shadow-xl max-w-lg w-full max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="p-5 overflow-y-auto space-y-3">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div><label htmlFor="editableGridNumbersTotal" className={labelClasses}>Total Cuadrícula <span className="text-red-500">*</span></label><input type="number" id="editableGridNumbersTotal" value={editableGridNumbersTotal} onChange={(e) => setEditableGridNumbersTotal(e.target.value)} className={inputClasses} min="10" max="500" step="1" required disabled={isLoading}/></div>
          </div>
