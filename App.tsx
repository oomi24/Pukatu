...
import type { User, Raffle, Buyer, RaffleNumber, MillionBagState, MillionBagNumber } from './types';
import { NumberStatus, PaymentMethod } from './types';
...
const initialMillionBag: MillionBagState = {
    isActive: true,
...
const App: React.FC = () => {
  ...
  const [millionBag, setMillionBag] = useState<MillionBagState>(initialMillionBag);
  ...
  useEffect(() => {
    ...
    if (savedUsers) {
      ...
      const robleUser: User = { username: 'Roble', password: 'Apamate.25', role: 'superadmin', phone: '04120000000' };
      const adminUser: User = { username: 'AdminManager', password: 'password123', role: 'admin', phone: '04121112233' };
    ...
    if (savedMillionBag) {
        ...
        if (!parsedBag.config) {
            parsedBag.config = initialMillionBag.config;
        }
  ...
  const handleUpdateMillionBag = (updatedMillionBag: MillionBagState) => {
    setMillionBag(updatedMillionBag);
  };

  const handleCreateRaffle = (newRaffleData: Omit<Raffle, 'id' | 'numbers' | 'winningNumber' | 'isActive'>) => {
