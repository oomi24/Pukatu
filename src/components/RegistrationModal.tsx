...
import type { RaffleItem, TicketRegistration, PaymentMethod } from '../types';
...
const RegistrationModal: React.FC<RegistrationModalProps> = ({ show, onClose, raffleItem, onAddTicketRegistration }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const representativeTicketNumberForDB = finalTicketNumbersStr.join(', '); 
    ...
    const newRegistration: TicketRegistration = {
      ...
      ticketNumber: representativeTicketNumberForDB, 
      registrationDate: new Date().toISOString(),
      paymentMethod: paymentMethodInput as PaymentMethod, // Ensure it's not empty
      paymentReference: paymentReferenceInput,
      paymentStatus: 'Pendiente', // Initial status
      managedByAdminId: raffleItem.managedByAdminId,
    };
    onAddTicketRegistration(newRegistration); 

    let ticketCostInfo = raffleItem.ticketCost !== undefined ? ` (Costo Total: $${(raffleItem.ticketCost * finalTicketNumbersStr.length).toFixed(2)})` : '';
    let ticketSelectionInfo = `Número(s) Adquirido(s): ${finalTicketNumbersStr.join(', ')}`;
    
    const messageParts = [
      `*Nuevo Registro para Sorteo: ${modalTitle}${ticketCostInfo}*`,
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-[100] p-4" onClick={isLoading ? undefined : onClose} role="dialog">
      <div className="bg-[#1e1442] text-white border border-purple-700/50 rounded-lg shadow-xl max-w-md w-full max-h-[95vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        <div className="p-5 border-b border-purple-600/30">
          <h5 className="text-lg font-semibold font-['Orbitron'] text-[#f7ca18]" id="registrationModalLabel">Registro para: <span className="text-white">{modalTitle}</span>
            {raffleItem.ticketCost !== undefined && (<span className="block text-xs text-yellow-300 mt-1">Costo: ${raffleItem.ticketCost.toFixed(2)} (por número)</span>)}
          </h5>
        ...
        <form onSubmit={handleSubmit} className="p-5 overflow-y-auto space-y-3 flex-grow">
