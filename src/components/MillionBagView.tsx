...
import type { Raffle, MillionBagNumber, Buyer } from '../core/types';
...
const MillionBagView: React.FC<MillionBagViewProps> = ({ millionBag, onUpdateMillionBag, onBack }) => {
  ...
  useEffect(() => {
    ...
    if (!millionBag.drawConfig) return;

    const checkConditions = () => {
        const dateConditionMet = (millionBag.drawConfig?.mode === 'date' || millionBag.drawConfig?.mode === 'hybrid') && new Date(millionBag.closeDate) < new Date();
        const salesConditionMet = (millionBag.drawConfig?.mode === 'sales' || millionBag.drawConfig?.mode === 'hybrid') && soldCount >= 1000;
    ...
    checkConditions();
  }, [soldCount, millionBag.drawConfig, millionBag.winningNumber, millionBag.closeDate]);
  ...
  const handlePurchase = (buyer: Buyer) => {
    ...
    const updatedNumbers = millionBag.numbers.map(n => 
      (n as MillionBagNumber).number === selectedNumber ? { ...n, status: NumberStatus.Pending, buyer } : n
    );
  ...
  if (!millionBag.isMillionBag) {
     return <div className="p-4 text-red-500">Error: Vista incorrecta para este tipo de sorteo.</div>
  }

  const numbers = millionBag.numbers as MillionBagNumber[];
  const winnerInfo = numbers.find(n => n.number === millionBag.winningNumber);
  ...
  const getTriggerText = () => {
    ...
    switch(millionBag.drawConfig?.mode) {
    ...
    <div className="bg-black text-white min-h-screen">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-1 space-y-6">
                 {(millionBag.drawConfig?.mode === 'date' || millionBag.drawConfig?.mode === 'hybrid') && (
