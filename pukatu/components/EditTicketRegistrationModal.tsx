...
const EditTicketRegistrationModal: React.FC<EditTicketRegistrationModalProps> = ({ show, onClose, ticketRegistration, onSave, isRobleAdmin }) => {
  ...
  const validateTicketNumber = (num: string): boolean => {
    if (ticketRegistration?.raffleId === 'quick_raffle') return true; // Not applicable for quick raffle
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!participantName.trim() || !participantWhatsApp.trim()) {
      setErrorMessage("Nombre y WhatsApp del participante no pueden estar vacíos."); return;
    }
    ...
    if (!paymentMethod || !paymentReference.trim()) {
        setErrorMessage("Método y referencia de pago son obligatorios."); return;
    }
    ...
    >
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="p-5 overflow-y-auto space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" id="regParticipantName" value={participantName} onChange={(e) => setParticipantName(e.target.value)} className={inputClasses} required disabled={isLoading}/>
            </div>
            <div>
              ...
              <input type="tel" id="regParticipantWhatsApp" value={participantWhatsApp} onChange={(e) => setParticipantWhatsApp(e.target.value)} className={inputClasses} required pattern="^\d+$" disabled={isLoading}/>
            </div>
          </div>

          {ticketRegistration.raffleId !== 'quick_raffle' && (
            <div>
              <label htmlFor="regTicketNumber" className={labelClasses}>Número(s) de Ticket <span className="text-red-500">*</span></label>
              <input type="text" id="regTicketNumber" value={ticketNumber} onChange={handleTicketNumberChange} className={`${inputClasses} tracking-wider text-center font-mono`} required disabled={isLoading} />
            </div>
          )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <select id="paymentMethodEdit" value={paymentMethod} onChange={(e) => setPaymentMethod(e.target.value as PaymentMethod | '')} className={selectClasses} required disabled={isLoading || !isRobleAdmin} >
             ...
             <div>
              ...
              <select id="paymentStatusEdit" value={paymentStatus} onChange={(e) => setPaymentStatus(e.target.value as PaymentStatus | '')} className={selectClasses} required disabled={isLoading || !isRobleAdmin} >
          ...
          <div>
            ...
            <input type="text" id="paymentReferenceEdit" value={paymentReference} onChange={(e) => setPaymentReference(e.target.value)} className={inputClasses} required disabled={isLoading} />
          </div>

          {!isRobleAdmin && <p className="text-xs text-yellow-400 mt-1">Solo el Super Admin (Roble) puede cambiar el estado o método del pago.</p>}

          {ticketRegistration.raffleId === 'quick_raffle' && typeof ticketRegistration.prizeWon === 'number' && (
            <div className="p-3 bg-black/20 rounded-md text-center">
                <p className={labelClasses}>Premio Sorteo Rápido</p>
                <p className="text-xl font-bold text-green-400">
                    {ticketRegistration.prizeWon > 0 ? `$${ticketRegistration.prizeWon.toFixed(2)}` : 'Sin Premio'}
                </p>
            </div>
          )}
        ...
        <div className="p-4 border-t border-purple-600/30 text-right mt-auto">
          <button type="button" onClick={onClose} disabled={isLoading} className="bg-gray-600 text-white font-semibold py-2 px-5 rounded-md hover:bg-gray-500 transition-colors">
            Cancelar
          </button>
        </div>
      </div>
