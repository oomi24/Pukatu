  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isMainRaffle) {
      ...
      if (!raffleItem.selectedGridNumbers || raffleItem.selectedGridNumbers.length === 0) {
        setSubmitMessage('Error: No se ha seleccionado ningún número. Por favor, cierre esta ventana y vuelva a intentarlo.');
        setSubmitMessageType('error');
        setIsLoading(false);
        return;
      }
      numberOfTickets = raffleItem.selectedGridNumbers.length;
      ticketNumberForDb = raffleItem.selectedGridNumbers.join(', ');
      if (raffleItem.ticketCost) {
        totalCost = raffleItem.ticketCost * numberOfTickets;
      }
    }
    ...
    let ticketCostInfo = totalCost !== undefined ? ` (Costo Total: $${totalCost.toFixed(2)} por ${numberOfTickets} ticket(s))` : '';
      ...
      <div className="bg-[#1e1442] text-white border border-purple-700/50 rounded-lg shadow-xl max-w-md w-full max-h-[95vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        <div className="p-5 border-b border-purple-600/30">
          <h5 className="text-lg font-semibold font-['Orbitron'] text-[#f7ca18]" id="registrationModalLabel">Registro para: <span className="text-white">{modalTitle}</span>
            {raffleItem.ticketCost !== undefined && (
                <span className="block text-xs text-yellow-300 mt-1">
                    Costo: ${raffleItem.ticketCost.toFixed(2)} {(raffleItem.raffleType === 'grid' ? `(por número) - Total: $${(raffleItem.ticketCost * (raffleItem.selectedGridNumbers?.length || 1)).toFixed(2)}` : '(por participación)')}
                </span>
            )}
          </h5>
        ...
        <form onSubmit={handleSubmit} className="p-3 sm:p-5 overflow-y-auto space-y-3 flex-grow">
          ...
          {isMainRaffle ? (
            ...
            <div>
                <p className={labelClasses}>Números Seleccionados:</p>
                <p className="text-lg font-['Orbitron'] text-[#f7ca18] text-center bg-black/20 p-2 rounded break-words">
                    {raffleItem.selectedGridNumbers.join(', ')}
                </p>
            </div>
          ) : (
            <div className="text-center p-3 bg-red-900/50 rounded-md">
                <p className="text-sm text-yellow-300">Error: No se han proporcionado números. Por favor, cierre esta ventana y vuelva a seleccionar sus números.</p>
            </div>
           )}
          ...
          <div className="pt-2">
            <button 
                type="submit" 
                disabled={isLoading || !raffleItem.administratorWhatsApp || !raffleItem.managedByAdminId || !paymentMethodInput || (raffleItem.raffleType === 'grid' && (!raffleItem.selectedGridNumbers || raffleItem.selectedGridNumbers.length === 0))} 
