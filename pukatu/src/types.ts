...
export interface RaffleItem {
  ...
  gridStartNumber?: number; // To allow grids that don't start at 1
  raffleTitle: string; 
...
export interface UserTicketsViewData {
    ...
    quickRafflePlays: TicketRegistration[]; // To display quick raffle results
}
...
export interface TicketRegistration {
  ...
  managedByAdminId: UUID; 
  prizeWon?: number; // For the quick raffle prize
  hasBeenPlayed?: boolean; // For the quick raffle play state
}