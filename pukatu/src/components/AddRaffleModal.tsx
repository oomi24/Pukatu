...
const AddRaffleModal: React.FC<AddRaffleModalProps> = ({ show, onClose, onAdd, adminId, adminWhatsApp }) => {
  ...
  const [gridStartNumber, setGridStartNumber] = useState<number | string>(1);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isNaN(parsedGridTotal) || parsedGridTotal <= 0 || parsedGridTotal > 1000) { 
        setErrorMessage("El total de números en cuadrícula debe ser un número positivo (ej: 100, máx 1000)."); return;
    }
    const parsedGridStartNumber = parseInt(String(gridStartNumber), 10);
    if (isNaN(parsedGridStartNumber) || parsedGridStartNumber < 0) {
        setErrorMessage("El número inicial debe ser un número positivo o cero.");
        return;
    }
      ...
      <div className="bg-[#1e1442] text-white border border-purple-700/50 rounded-lg shadow-xl max-w-lg w-full max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="p-5 overflow-y-auto space-y-3">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              <label htmlFor="ticketCost" className={labelClasses}>Costo Ticket ($) <span className="text-red-500">*</span></label>
              <input type="number" id="ticketCost" value={ticketCost} onChange={(e) => setTicketCost(e.target.value)} className={inputClasses} placeholder="Ej: 1.00" min="0" step="0.01" required disabled={isLoading}/>
            </div>
            <div>
              <label htmlFor="gridNumbersTotal" className={labelClasses}># Total <span className="text-red-500">*</span></label>
              <input type="number" id="gridNumbersTotal" value={gridNumbersTotal} onChange={(e) => setGridNumbersTotal(e.target.value)} className={inputClasses} placeholder="Ej: 100" min="10" max="1000" step="1" required disabled={isLoading}/>
            </div>
            <div>
              <label htmlFor="gridStartNumber" className={labelClasses}># Inicial <span className="text-red-500">*</span></label>
              <input type="number" id="gridStartNumber" value={gridStartNumber} onChange={(e) => setGridStartNumber(e.target.value)} className={inputClasses} placeholder="Ej: 1" min="0" step="1" required disabled={isLoading}/>
            </div>
          </div>
